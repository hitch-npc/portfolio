...
export const PARAMS = {
  ...
  tracking: 0.165,     // разрядка знака, доли кегля
  fit: 0.54,           // доля ширины холста под знак
  ...
  rowsPerCap: 14,      // сколько рядов фигур укладывается в высоту прописной
  cellMin: 6,
  cellMax: 34,
  ...
  blurRatio: 0.08,     // размытие от шага сетки — мягкость края держится пропорционально
  shape: 'triangle',   // фигура в ячейке (see shapes.js); её размеры — там же
  ghost: 0.055,        // покрытие вне буквы: еле видные фигуры фона
  weight: 2.7,         // толщина линии для линейных фигур, css-пиксели
  flowScale: 7.2,      // масштаб поля потока: меньше — крупнее и глаже волны
  flowSpeed: 0.06,     // скорость дрейфа поля
  breathe: 1,          // глубина инверсии: 0 — выключено, 1 — полный негатив в пятне
  breatheScale: 2.8,   // масштаб пятен инверсии: меньше — пятна крупнее
  breatheSpeed: 0.22,  // скорость дрейфа пятен: своё время, медленнее потока
  pulse: 0.55,         // вдох-выдох: насколько фигуры растут и мельчают волной, доли размера
  pulseScale: 1.9,     // длина волны вдоха по знаку: меньше — волна шире
  pulseSpeed: 0.7,     // темп дыхания, рад/с: 0.7 — вдох и выдох примерно за 9 с
  mouseRadius: 600,    // радиус вихря, css-пиксели
  swirl: 0.98,         // насколько курсор перебивает поле (0..1)
  magnify: 0.08,       // лупа под курсором: ячейки расходятся от центра и крупнеют
  push: 2,             // прежнее расталкивание, css-пиксели: >0 пробивает дыру под курсором
  invert: 0.82,        // глубина линзы под курсором: 0 — нет, 1 — полный негатив
  invertRadius: 380,   // радиус линзы, css-пиксели: меньше вихря — линза внутри него
  ease: 0.08,          // инертность курсора
...
export class GlyphField {
  ...
  constructor(canvas, params = {}) {
    ...
    // Размер следим у самого холста, а не у окна: страница может собраться,
    // пока холст нулевой (скрытая вкладка, панель превью), и знак остался бы
    // пустым до первого ресайза окна. Наблюдатель отвечает и на первый размер.
    // Тот же размер — пропуск: пересборка маски и сетки дорогая. Размер берём
    // у наблюдателя: он не видит transform, которым hero надвигается при прокрутке
    this._onResize = ([entry]) => {
      const box = entry.contentRect;
      if (Math.round(box.width) === this.w && Math.round(box.height) === this.h) return;
      this.resize();
    };
    this._sizer = new ResizeObserver(this._onResize);

    window.addEventListener('pointermove', this._onPointer, { passive: true });
    window.addEventListener('blur', this._onLeave);
    this._sizer.observe(canvas);
  }
